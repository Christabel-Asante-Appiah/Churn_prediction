...
st.write("Enter Accounnt Transaction details and click **Predict Churn**.")
st.divider()

#defining data inputs for the app from the model trained
...
def build_feature_df():
    ...
    # Ensuring features are numeric data types
...
if st.button("Predict Churn"):
    try:
        ...
        if hasattr(scaler, "n_features_in_") and scaler.n_features_in_ != X_df.shape[1]:
            st.warning(f"Scaler expects {scaler.n_features_in_} features, but you provided {X_df.shape[1]}. Check FEATURE_COLUMNS/order.")
        if hasattr(model, "n_features_in_") and model.n_features_in_ != X_df.shape[1]:
            st.warning(f"Model expects {model.n_features_in_} features, but you provided {X_df.shape[1]}. Check FEATURE_COLUMNS/order.")
